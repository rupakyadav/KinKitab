...
/**
 * Public-facing card used on the marketplace feed.
 * Sold listings are dimmed and clearly labeled.
 */
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-stone-100">
        ...
        {isSold && (
          <div className="absolute inset-0 flex items-center justify-center bg-stone-900/45">
            <span className="rounded-md bg-white/95 px-3 py-1 text-xs font-bold uppercase tracking-wider text-stone-800">
              Sold
            </span>
          </div>
        )}
        <span className="absolute right-3 top-3 rounded-md bg-white/95 px-2 py-0.5 text-xs font-medium text-stone-700 shadow-sm">
          {conditionLabel(listing.condition)}
        </span>
      </div>

      <div className="flex flex-1 flex-col p-4">
        <h3 className="line-clamp-2 font-semibold text-stone-900">{listing.title}</h3>
        ...
        <div className="mt-3 flex items-end justify-between">
          ...
          {sellerLocation && (
            <span className="inline-flex max-w-[55%] items-center gap-1 truncate text-xs text-stone-500">
